...
void setup() {
  size(224, 176);
  pc = new PixalingClass("casaJupiter.jpg", width/div, height/div, true /* resize */, /*colors*/ "colors.csv");
...
void draw() {
  ...
  //float size = float(width)/colors.length;
  //float x = 0;
  //for (int i = 0; i < colors.length; ++i) {
  //  fill(colors[i]);
  //  rect(x, height/2.0, x+size, 50+(height/2.0));
  //  x += size;
  //}
}
...
void mousePressed() {
  ...
  println(pc.getColor(mouseX, mouseY), pc.colorToString(pc.getClosestColor(pc.getColorVal(mouseX, mouseY))));
  fill(pc.getClosestColor(pc.getColorVal(mouseX, mouseY)));
  rect(mouseX-10, mouseY-10, mouseX, mouseY);
}
